...
public class SimilarInquiryService {
        ...
        for (Inquiry inquiry : recentInquiries) {
            ...
            if (similarity >= SIMILARITY_THRESHOLD) {
                results.add(new SimilarInquiry(
                        inquiry.getId(), inquiry.getMessage(),
                        inquiry.getStatus().name(), similarity
                ));
            }
    ...
    public record SimilarInquiry(Long inquiryId, String message, String status, double similarity) {}
}
